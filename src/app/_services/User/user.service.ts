...
export class UserService {
  private user: any;
  private subject: BehaviorSubject<any> = new BehaviorSubject(this.user);
  ...
  constructor(private http: Http, 
    ...
    this.subject.subscribe(data => this.user = data);    
  ...
  create(user: any) {
    user.Imagem = this.getDefaultUserImage();

    return this.http.post(this.config.apiUrl + '/api/users', JSON.stringify(user), this.options);
  }

  update(user: any) {
    return this.http.put(this.config.apiUrl + '/api/users', JSON.stringify(user), this.options);
  }
